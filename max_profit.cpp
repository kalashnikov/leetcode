...
class cad {
  public:
    ...
    cad( int b, int s, int p):buy(b),sell(s),profit(p) {}

    //static bool cmp(const cad&a, const cad& b){
...
int maxProfit2(vector<int>& prices) {
  ...
  for(int i=i2+1;i<prices.size();i++){
    if(prices[i]-m2>p2)
      p2 = prices[i]-m2;
  }
...
int maxProfit1(vector<int>& prices) {
  ...
  if ( prices.size()>100 ) { 
    ...
    for(int i=1;i<prices.size();i++){
      ...
      if ( isRev && i > prices.size()/10 ) return 0;
    }
    if (isRev) return 0;
  }
  ...
  int threshold = (maxv-minv)*0.95;

  for(int i=0;i<prices.size()-1;i++){
    for(int j=i+1;j<prices.size();j++){
      if ( prices[i] > prices[j] ) continue;
      if ( prices.size() > 100 && prices[j] - prices[i] < threshold ) continue;
      cad c(i,j,prices[j]-prices[i]);
      if ( debug ) cout << i << "," << j << " -> " << prices[j]-prices[i] << endl;
      cadlist.push_back(c);
    }
  }

  if ( cadlist.size()==0 ) return 0;

  int max = 0;
  for(int i=0;i<cadlist.size()-1;i++){
    if ( debug )cout << i << " | " << prices[cadlist[i].buy] << " " << prices[cadlist[i].sell] << " -> " << cadlist[i].profit << endl;

    for(int j=i+1;j<cadlist.size();j++){
      if( ( (cadlist[i].buy < cadlist[j].buy && cadlist[i].sell < cadlist[j].buy ) ||
            (cadlist[j].buy < cadlist[i].buy && cadlist[j].sell < cadlist[i].buy ) ) &&
          ( cadlist[i].profit + cadlist[j].profit ) > max ) {
        max = cadlist[i].profit + cadlist[j].profit;
        if ( debug ) cout << i << " and " << j << " -> " << max << endl;
      }
    }
  }
...
int main(){
  ...
  vector<int> v0 {1,2,4,7};
  for(int i=0; i<v0.size(); i++){
    cout << v0[i] << " ";
  }
  cout << endl;
  cout << maxProfit(v0) << endl << endl;

  vector<int> v1 {5,9,8,1,7,2,10,3};
  for(int i=0; i<v1.size(); i++){
    cout << v1[i] << " ";
  }
  cout << endl;
  cout << maxProfit(v1) << endl << endl;
  
  //vector<int> v2 {2,1};
  //cout << maxProfit(v2) << endl;

  vector<int> v3 {2,1,2,0,1};
  for(int i=0; i<v3.size(); i++){
    cout << v3[i] << " ";
  }
  cout << endl;
  cout << maxProfit(v3) << endl << endl;

  vector<int> v4 {2,1,4,5,2,9,7};
  for(int i=0; i<v4.size(); i++){
    cout << v4[i] << " ";
  }
  cout << endl;
  cout << maxProfit(v4) << endl << endl;
